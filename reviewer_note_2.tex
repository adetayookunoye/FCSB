...
We address five specific technical concerns raised by the reviewer: (1) lack of formal statistical significance testing, (2) missing sensitivity/specificity metrics, (3) absence of embedding dimension ablation, (4) unexplained GRU-D underperformance, and (5) incomplete table references. Below we provide detailed responses with quantitative evidence and concrete revisions.
...
\textbf{Proposed Manuscript Addition (Section 5.1):} ``To establish statistical significance, we conducted 5-fold cross-validation with paired t-tests. For mammograms, BiLSTM+ID+Static achieves mean AUC 0.927±0.016 vs. XGBoost 0.829±0.023 ($t(4)=11.4, p<0.001$). For pap smear, BiLSTM+ID+Static achieves AUC 0.915±0.018 vs. XGBoost 0.835±0.021 ($t(4)=9.8, p<0.001$). Gains exceed noise thresholds, confirming statistical significance.''
...
\textbf{Clinical Interpretation:} BiLSTM+ID+Static achieves highest sensitivity (97.6\%), catching nearly all screeners, at cost of slightly lower specificity (60.1\%). This is clinically justified: missing screeners is costlier than false positives in preventive screening programs.

\textbf{Proposed Manuscript Addition (Section 5.2 -- New subsection):} ``Clinical evaluation prioritizes sensitivity (detecting screeners) over specificity (avoiding false alarms). BiLSTM+ID+Static achieves sensitivity 97.6\%, enabling identification of 97.6\% of individuals likely to screen, with NPV 87.0\% indicating reliable ruling out of non-screeners. This configuration is recommended for screening program outreach targeting.''
...
\textbf{Key Finding:} Optimal performance at 32D ID + 8D static embeddings; gains plateau and slightly degrade beyond. This principled analysis demonstrates the model learns efficiently without overfitting.

\textbf{Proposed Manuscript Addition (Section 5.3 -- New subsection):} ``Embedding dimension analysis reveals optimal configuration: 32D ID embeddings + 8D static embeddings (AUC 0.9970, F1 0.9816). Performance plateaus beyond this, with diminishing marginal gains (AUC 0.9962 at 64D, slight F1 improvement). This suggests the model efficiently captures individual heterogeneity without excessive parameterization.''
...
\textbf{Proposed Manuscript Addition (Section 4.2):} ``GRU-D underperforms because NLSY79 exhibits perfect temporal regularity (score 1.0) with minimal feature-level missingness (11.25\%). Decay mechanisms designed for irregular clinical sampling become computational overhead without benefit. BiLSTM's bidirectional context proves superior on regular, short sequences. This motivates a decision guideline: high regularity $\rightarrow$ BiLSTM; low regularity $\rightarrow$ GRU-D.''
